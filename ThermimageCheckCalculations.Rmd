---
title: "Thermimage: Calculation Checks"
...
The purpose of this script is to compare Thermimage's raw2temp() conversion calculations in the 
...
The plot above is simply the raw binary radiance values imported. The scale should be in raw units
where max possible = 2^16=65535 corresponding to the data storage method from FLIR jpgs)
...
Below, I load in 3 files that represent the same image set to realistic emissivity and environmental
values, calculated within each program (Thermacam Researcher Pro v2.9, ExaminIR, and Flir Research R&D Max), and compare mean values (across all 307200 pixels):
```{r}
